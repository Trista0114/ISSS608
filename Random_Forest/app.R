...
Property_data <- read.csv("C:/Trista0114/ISSS608/Random_Forest/data/Property_Price_and_Green_Index.csv")
  ...
  fluidRow(
    ...
    column(
      ...
      sidebarPanel(
        ...
        numericInput("seed", "Set seed (0-9999)", value = 1234, min = 0, max = 9999),
        sliderInput("train_ratio", "Train-Test Partition Ratio", 
                    min = 0.5, max = 0.95, value = 0.8, step = 0.05),
        sliderInput("num_trees", "Num of Trees", 
        ...
        selectInput("split_rule", "Split rule", 
                    ...
                    selected = "variance"),
        selectInput("feature_importance", "Feature Importance", 
    ...
    column(
      ...
      mainPanel(
          ...
          column(width = 12,
                 fluidRow(
                   ...
                   column(width = 4,
                          ...
                          plotOutput("oob_error", height = "300px")
                   ),
                   column(width = 4,
        ...
        fluidRow(
          column(width = 3,
                 wellPanel(
                   div(strong("Test R-squared:"), textOutput("test_r2")),
                   div(strong("Test MSE:"), textOutput("test_mse")),
                   div(strong("Train R-squared:"), textOutput("train_r2")),
                   div(strong("Train MSE:"), textOutput("train_mse"))
                 )
          ),
          column(width = 9,
                 h3("Suggested setting/Thought"),
                 textOutput("suggested_setting")
          )
        )
      )
    )
...
server <- function(input, output, session) {
  # Reactive value to store the model and predictions
  model_data <- reactiveVal(NULL)
  
  # Build model when button is clicked
  observeEvent(input$build, {
    # Update progress bar step-by-step
    updateProgressBar(session, id = "incProgress", value = 0, total = 100, title = "Starting model training")
    
    # 1. Set random seed
    set.seed(input$seed)
    updateProgressBar(session, id = "incProgress", value = 10, title = "Set the seed")
    ...
    updateProgressBar(session, id = "incProgress", value = 20, title = "Splitting data")
    ...
    updateProgressBar(session, id = "incProgress", value = 30, title = "Building formula")
    ...
    updateProgressBar(session, id = "incProgress", value = 40, title = "Setting parameters, good model is worth to wait")
    ...
    updateProgressBar(session, id = "incProgress", value = 70, title = "Training model")
    ...
    updateProgressBar(session, id = "incProgress", value = 90, title = "Calculating metrics")
    ...
    updateProgressBar(session, id = "incProgress", value = 100, title = "Done, congrats!")
  })
