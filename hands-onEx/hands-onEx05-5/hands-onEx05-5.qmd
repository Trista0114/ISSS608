...
-   value mode (for diverging palettes, such as "RdYlBu"):
    ...
    -   0 → intermediate color (usually white/yellow)
    -   -max(abs(values)) → left end color
    -   max(abs(values)) → right end color

-   manual mode:

    -   min(values) → left end color

    -   max(values) → right end color

    -   mean(range(values)) → middle color

Simply, value is suitable for divergent color mapping, while manual corresponds to color linearly! 🚀
:::

## 3.4 The “value” type treemap
...
✅ **Recommended**: Keeps rectangles close to square shape (default)
...
Use “pivotSize” with *sortID*  to dertemine the order in which the rectangles are placed from top left to bottom right.
