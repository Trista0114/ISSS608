...
```{r}
pacman::p_load(ggplot2,dplyr,scales, viridis, lubridate, ggthemes, gridExtra, readxl, knitr, data.table, CGPfunctions, ggHoriPlot, tidyverse)
```
...
Please examine the imported data frame before further analysis is performed.
...
## Step2:Preparing the tidy data frame 
...
## Step3: Plotting the Multiple Calender Heatmap by using ggplot2 package
...
## Step2: **Deriving month and year fields**
...
## Step3: **Extracting the target country and time period**

Next, the code chunk below is use to extract data for the target country (i.e. Vietnam) and the year\>2010.
...
## Step4: **Computing year average arrivals by month**
...
-   `Year`：X axis → 1961 and 1980。

-   `Yield`：Y axis (rice production)。

-   `Country`：every line represent one country.
